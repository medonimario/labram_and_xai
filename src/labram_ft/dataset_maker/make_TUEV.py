...
root = os.path.join(raw_dataset_path, "tuh_eeg_events/v2.0.0/edf")

processed_dataset_path = os.getenv("PROCESSED_DATASET_PATH")
train_out_dir = os.path.join(processed_dataset_path, "tuh_eeg_events/processed_train")
eval_out_dir = os.path.join(processed_dataset_path, "tuh_eeg_events/processed_eval")
...
# root = "/share/TUEV/"
root_processed_train = train_out_dir  # $PROCESSED_DATASET_PATH/tuh_eeg_events/processed_train
root_processed_eval  = eval_out_dir   # $PROCESSED_DATASET_PATH/tuh_eeg_events/processed_eval

final_root = os.path.join(processed_dataset_path, "tuh_eeg_events/processed")
os.makedirs(os.path.join(final_root, "processed_train"), exist_ok=True)
os.makedirs(os.path.join(final_root, "processed_eval"),  exist_ok=True)
os.makedirs(os.path.join(final_root, "processed_test"),  exist_ok=True)


seed = 4523
np.random.seed(seed)

train_files = os.listdir(root_processed_train)
train_sub = list(set([f.split("_")[0] for f in train_files]))
print("train sub", len(train_sub))
test_files = os.listdir(root_processed_eval)
...
for file in train_files:
    shutil.copy2(os.path.join(root_processed_train, file),
                 os.path.join(final_root, "processed_train", file))

for file in val_files:
    shutil.copy2(os.path.join(root_processed_train, file),
                 os.path.join(final_root, "processed_eval", file))

for file in test_files:
    shutil.copy2(os.path.join(root_processed_eval, file),
                 os.path.join(final_root, "processed_test", file))