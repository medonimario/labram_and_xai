...
if __name__ == "__main__":
    ...
    folds = []
    for _, test_idx in kf.split(all_pair_ids):
        folds.append(all_pair_ids[test_idx])

    print(f"Total pairs: {len(all_pair_ids)}")

    final_root = os.getenv("CIRCLING_PROCESSED_PATH", os.path.join(base_path, "processed_cv/"))
    parameters = []

    # Create fold_0, fold_1 ... fold_9 and prepare parameters
    ...
    for fold_idx, fold_pairs in enumerate(folds):
        ...
        fold_files = [f for pid in fold_pairs for f in pair_files[pid]]
        print(f"Fold {fold_idx} pairs: {fold_pairs}")
