...
def get_label(cb_cond, video_num):
    """
    Assigns binary label based on Counterbalance condition and video number.
    Label 0: Low Noise
    Label 1: High Noise
    """
    if cb_cond == 'A':
        return 0 if video_num in [3, 4, 5, 6] else 1
    elif cb_cond == 'B':
        return 1 if video_num in [3, 4, 5, 6] else 0
    else:
        raise ValueError(f"Unknown counterbalance condition: {cb_cond}")
...
def process_npz_file(params):
    ...
    """
    filepath, dump_folder, split_name, entity_id, cb_cond = params
    ...
    try:
        ...
        # 2. Get Label
        label = get_label(cb_cond, video_num)
...
def main():
    ...
    final_root = os.path.join(ENGAGEMENT_DATASET_PATH, "processed_noise")
    ...
    for root, _, files in os.walk(base_path):
        ...
        for file in files:
            if file.endswith('.npz'):
                ...
                # Determine Counterbalance Condition (A or B)
                match_cb = re.search(r'_([AB])_', folder_name)
                if not match_cb:
                    print(f"Could not find counterbalance (A/B) in {folder_name}. Skipping.")
                    continue
                    
                cb_cond = match_cb.group(1)
                entity_files[entity_id].append((filepath, cb_cond))
    ...
    for split_name, (entities, dump_folder) in splits.items():
        ...
        for entity in entities:
            for filepath, cb_cond in entity_files[entity]:
                parameters.append([filepath, dump_folder, split_name, entity, cb_cond])
